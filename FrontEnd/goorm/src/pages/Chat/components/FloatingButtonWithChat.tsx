...
import { getPrivateChatRoomsByMember, getChatHistory, getPublicChatRoomsByMember, getPublicChatRoom } from '../api/chatApi';
...
const FloatingButtonWithChat: React.FC = () => {
  ...
  const closeModal = () => {
    ...
    const sendJoinMessage = () => {
      ...
      clientRef.current.send(`/api/pub/join/${currentChatRoomId}`, {}, JSON.stringify(message));
    }    
  ...
  useEffect(() => {
    if (inChatRoom === true) {
      ...
      //웹소켓 연결 설정
      const socket = new SockJS("http://localhost:8080/api/websocket", {withCredentials:true});
      ...
      client.connect({}, () => {
        ...
        client.subscribe(`/api/sub/join/${currentChatRoomId}`, (message) => {
          // console.log('message:',message)
          const newMessage = JSON.parse(message.body);
  ...
  const renderChatRoomList = () => (
    <ul className="chat-room-list">
      {chatRooms.map((chatRoom) => (
        <li key={chatRoom.chatRoomId} onClick={() => enterChatRoom(chatRoom)}>
  ...
  //오픈 채팅방 리스트 렌더링 (채팅방 입장 전 모달창으로 입장확인)
          ...
          <div className={`chat-message ${message.sender === '구름이' ? 'right' : 'left'}`} key={message.id}>
            {message.sender === '구름이' && (
              <div className="message-info">
                ...
                <span className="message-time">{formatTime(message.sendTime)}</span>
              </div>
            )}
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      case 'home':
        return <div className="chat-body">{renderHomeList()}</div>;
      case 'chat':
      ...
      {isChatOpen && (
        <div className="chat-window">
          ...
          <Modal
            ...
            parentSelector={() => document.querySelector('.chat-window') || document.body}
          >
            <h2>알림</h2>
            <div className="modal-content">
              <p>채팅방에 입장하시겠습니까?</p>
              <div className="modal-buttons">
                <button className="confirm-button" onClick={confirmEnterChatRoom}>확인</button>
